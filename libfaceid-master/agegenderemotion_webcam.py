...
def process_facedetection(model_detector, model_ageestimator, model_genderestimator, model_emotionestimator, cam_resolution, cam_index):
    ...
    try:
        ...
        face_detector = FaceDetector(
            model=model_detector, path=INPUT_DIR_MODEL_DETECTION, minfacesize=120)
...
#            model = model_poseestimator, path = INPUT_DIR_MODEL_ESTIMATION)
        face_age_estimator = FaceAgeEstimator(
            model=model_ageestimator, path=INPUT_DIR_MODEL_ESTIMATION)
...
#            face_pose_estimator.add_overlay(frame, shape)
        ...
        if keyPressed == 27:  # ESC
            ...
            cv2.imwrite(
                WINDOW_NAME + "_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S") + ".jpg", frame)
...
def main(args):
    ...
    resolutions = [RESOLUTION_QVGA, RESOLUTION_VGA,
                   RESOLUTION_HD, RESOLUTION_FULLHD]
    try:
